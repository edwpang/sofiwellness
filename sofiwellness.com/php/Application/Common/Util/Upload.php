...
{     
    ...
    private $FileName; 
    /** 
     *@private string contains the temporary name of the file to be uploaded. 
     */ 
    ...
    //if set, then save the uploaded file to this name.
    ...
    { 
        ...
        if ($MaximumFileSize <= $TempFileSize) { 
            $this->SetMessage("ERROR: The file file is too big. It must be less than $MaximumFileSize and it is $TempFileSize."); 
            return false; 
        } 
    ...
    { 
        ...
        if (file_exists($File)) { 
            ...
            unlink ($File);
            $this->SetMessage("delete file");
            if (file_exists($File))
            {
            	
            	$this->SetMessage("cannot delete file, generate rand file name");
    ...
    { 
        ...
        if (substr($UploadDirectory, -1) != "/") { 
            $this->SetMessage("ERROR: The traling slash does not exist:" .$UploadDirectory); 
        ...
        if (!$this->ValidateExtension()) { 
            ...
            {  
            	...
            	if ($this->newFileName != null)
            		$fName = $this->newFileName;
            		
              	move_uploaded_file($TempFileName, $UploadDirectory . $fName); 
                return true; 
            } else { 
                return false; 
            } 
